...
<template>
  <div class="project-card" @click="goToProject">
    <div class="project-card-image">
      <!-- <img :src='assetUrl + project.name_slug + "-thumbnail.jpg"'
                :alt='project.name_slug + "-thumbnail"'> -->
      <img :src="getThumbnailUrl()" :alt="project.name_slug + '-thumbnail'" />
    </div>
...
export default {
  ...
  data() {
    return {
      projectTags: this.project.categories_name.filter(
        (tag) => tag !== "Tout voir"
      ),
    };
  },
  methods: {
    goToProject() {
      this.$router.push({
        name: "project",
        params: { name: this.project.name_slug },
      });
    },
    getThumbnailUrl() {
      let url;
